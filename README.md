...
Cloudflare Workers 反向代理：把 Cline 返回的 `delta.reasoning` 重写为 `delta.reasoning_content`，使上游（sub2api）能正确识别出 thinking 内容，转成 Anthropic thinking block。

## 工作原理

- 原样转发请求到 OpenA 兼容的 chat/completions 接口（默认 `https://api.cline.bot/api/v1/chat/completions`），`model` / `messages` 均不改动。
...
- 鉴权：优先使用环境变量 `CLINE_API_KEY`，未设置则透传请求自带的 `Authorization`。
...
把客户端（如 CC Switch / sub2api）的 base URL 指向本 Worker 的地址，直接发标准的 chat/completions 请求即可。Worker 只接受 `POST`。